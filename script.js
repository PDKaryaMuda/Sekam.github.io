document.addEventListener('DOMContentLoaded', function() {
    ...
    today = dd + '/' + mm + '/' + yyyy;
    document.getElementById('tanggalHariIni').innerHTML = '<p>Tanggal hari ini: <br><br> ' + today + '</p>';
 ...
 document.getElementById('jadwalPengambilan').innerHTML = 'Tanggal :' + jadwalFormatted + '<br> Pengambilan sekam <br> <b>Grup ' + group + '</b></p>';
    
    // Pemberitahuan jika hari ini adalah tanggal pengambilan barang
    if (today === jadwalFormatted) {
        document.getElementById('pemberitahuan').innerHTML = '<p>Hari ini:<br><br> <b>>> JADWAL PENGAMBILAN SEKAM <<</b></</p>';
    } else {
        ...
        var nextJadwalFormatted = String(nextJadwalPengambilan.getDate()).padStart(2, '0') + '/' + String(nextJadwalPengambilan.getMonth() + 1).padStart(2, '0') + '/' + nextJadwalPengambilan.getFullYear();
         ...
         if (today !== jadwalFormatted && today !== nextJadwalFormatted) {
            document.getElementById('jadwalPengambilan').innerHTML = '<p>Jadwal hari ini:<br><br><b>Libur</b></p>';
        } else {
            document.getElementById('jadwalPengambilan').innerHTML = '<p>Jadwal pengambilan sekam: ' + jadwalFormatted + '<br> (Grup ' + group + ')</p>';
        }
            // Jika hari ini bukan tanggal pengambilan barang, tampilkan jadwal pengambilan selanjutnya
            if (today !== jadwalFormatted && today !== nextJadwalFormatted) {
                document.getElementById('pemberitahuan').innerHTML = '<p>Jadwal selanjutnya: <br><br> <b>Grup ' + nextGroup + ' tanggal ' + nextJadwalFormatted + ' </b></p>';
            } else {
                document.getElementById('pemberitahuan').innerHTML = '<p>Jadwal pengambilan sekam: ' + jadwalFormatted + ' (Grup ' + group + ')</p>';
            }
    }
});